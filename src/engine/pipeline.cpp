...
void Pipeline::createGraphicsPipeline(const std::string& shader) {
    ...
    shaderStages[0].module = createShaderModule(readFile("engine/shaders/" + shader + ".vert.spv"));
    shaderStages[0].pName = "main";
    shaderStages[1].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    shaderStages[1].stage = VK_SHADER_STAGE_FRAGMENT_BIT;
    shaderStages[1].module = createShaderModule(readFile("engine/shaders/" + shader + ".frag.spv"));
...
void Pipeline::createSprite(std::shared_ptr<Model> model, unsigned char textureIndex, float positionx, float positiony, float scalex, float scaley, float rotation, float r, float g, float b, float a) {
...
void Pipeline::loadAstroids() {
    texturePaths = {  };
    fonts = {};
    loadSprites();

    createSprite(squareModel, 0, 0.f, 0.f, .05f, .05f, 0.f, 1.f, 1.f, 1.f, 1.f);
}