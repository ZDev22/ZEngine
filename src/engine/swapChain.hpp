...
struct SwapChain {
public:
    inline SwapChain(Device& deviceRef, VkExtent2D extent) : device{ deviceRef }, windowExtent{ extent } { init(); }
    inline SwapChain(Device& deviceRef, VkExtent2D extent, std::shared_ptr<SwapChain> previous) : device{ deviceRef }, windowExtent{ extent }, oldSwapChain{ previous } { init(); oldSwapChain = nullptr; }
    inline ~SwapChain() {
    ...
    inline VkResult submitCommandBuffers(const VkCommandBuffer* buffers, unsigned int* imageIndex) {
    ...
    inline void createSwapChain() {
        ...
        if (vkCreateSwapchainKHR(device.device(), &createInfo, nullptr, &swapChain) != VK_SUCCESS) { throw("failed to create swap chain!"); }
    ...
    inline void createImageViews() {
        ...
        for (unsigned int i = 0; i < swapChainImages.size(); i++) {
            ...
            if (vkCreateImageView(device.device(), &viewInfo, nullptr, &swapChainImageViews[i]) != VK_SUCCESS) { throw("failed to create texture image view!"); }
        }
    }
    ...
    inline unsigned int height() const { return swapChainExtent.height; }
    inline float extentAspectRatio() const { return (float)(swapChainExtent.width) / static_cast<float>(swapChainExtent.height); }
    inline bool compareSwapFormats(const SwapChain& swapChain) const { return swapChain.swapChainDepthFormat == swapChainDepthFormat && swapChain.swapChainImageFormat == swapChainImageFormat; }
