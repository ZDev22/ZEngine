...
inline int std::vectorContainsString(const char* target, const char** list, int listSize) { for (int i = 0; i < listSize; i++) { if (charCompare(list[i], target) == 0) return i; } return -1; }
